...
\begin{proof}[Proof of $ \QC_{m,n} \subset M $]
	...
	\begin{align*}
		\beta(S_1+S_2,T) = \textup{Re}(\trace{\rho(S_1+S_2)T}) = \textup{Re}(\trace{\rho S_1T}) + \textup{Re}(\trace{\rho S_2T}) = \beta(S_1,T)+ \beta(S_2+T).
	\end{align*}
	Symmetry follows form 
	\begin{align*}
		\beta(S,T)= \textup{Re}\trace{\rho ST} =\textup{Re}\trace{(\rho ST)^*}= \textup{Re \trace{T^*S^* \rho^*}} = \textup{Re} \trace{\rho^* T^*S^*} = \textup{Re} \trace{\rho T S} = \beta(T,S),
	\end{align*}
