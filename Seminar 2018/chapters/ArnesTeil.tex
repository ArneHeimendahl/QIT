...
\begin{proof}[Proof of $ \LC_{m,n} \supset M $]
	...
	Then $ \trace{(XY^T)} = d\cdot \langle x, y \rangle  $ and $ \norm{X}_{\infty} \le 1 $ since $ X_iX_i^* = \modul{x_i}^2I $ and $ \modul{x_i}^2 \le 1  $. The same holds for $ Y^T $ which also implies $ \norm{Y}_{\infty}  \le 1 $. \textbf{Elaborate}
