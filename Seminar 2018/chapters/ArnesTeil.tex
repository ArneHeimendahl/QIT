...
With regard to quantum information theory it is interesting to analyze the geometry of $ \LC_{m,n} $ and $ \QC_{m,n} $. As we will see in the following two lemmata, both sets can be described in a more useful way. 
...
1 & 1 \\ 1 & -1 
\end{pmatrix} $. Since $ \max \{  1/2 \trace{AM}\, | \, A \in \LC_{2,2} \} = 1 $ it suffices to show that there is $ A \in \QC_{m,n} $ achieving a better value. For $ A \in \QC_{2,2} $ we obtain, again by Cauchy-Schwarz and $ \modul{y_i}\le 1 $,
\begin{align*}
