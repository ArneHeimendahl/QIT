...
\subsection{Grothendieck's Inequality}
	...
	Now that we have proven the tools needed for Grothendieck's ineqaulity, we can give a definition of the program $\norm{M}_{\infty\to 1}$.
	...
	\begin{rmk}
		According to the current state of science $1.676\leq K_G \leq \beta^{-1} \approx 1.783$.
	\end{rmk}
...
\subsection{Grothendieck-Tsirelson}
	...
	\begin{theo}[Grothendieck-Tsirelson]
		...
		\begin{itemize}
			\item[(1)] We have the inclusion 
				...
				\end{equation}
			\item[(2)] For any $M\in\mathbb{R}^{m\times n}$ and for any $\rho,X_i,Y_j$ verifying the conditions of Definition \ref{dfn:QC} we have
				\begin{align}
					\sum_{i,j} M_{ij} \trace{\rho(X_i\tensor Y_j)} &\leq K \max_{\xi\in\{-1,1\}^m,\eta\in\{-1,1\}^n} \sum_{i,j} M_{ij}\xi_i\eta_j \\
					\Leftrightarrow \hspace{2.5cm} \trace{MA^\top} &\leq K \max_{\xi\in\{-1,1\}^m,\eta\in\{-1,1\}^n} \trace{M(\xi\eta^\top)^\top}.
				\end{align}
	...
	\begin{proof}
		...
		\begin{equation*}
			\trace{MA^\top} \geq c \text{ and } \trace{M(\xi\eta^\top)^\top} < c
		\end{equation*}
		for all $\xi\in\{-1,1\}^m,\eta\in\{-1,1\}^n$.
	\end{proof}
	
	This result gives us that the ratio of which an entangled strategy can exceed a classical strategy is bounded from above by $K$, such that an entangled strategy can not be arbitrary better than a classical strategy. 
\newpage