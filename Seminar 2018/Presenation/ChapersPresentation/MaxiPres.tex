...
\subsection{Grothendieck's Inequality}
	...
	\begin{frame}
		...
		\begin{pbmr}
			\begin{itemize}
				\item<2-> If $x$ and $y$ are linearly dependent, then
					\begin{itemize}
						\item<3-> if $x=y$:~~\, $\arccos(\sclr{x}{y})=\arccos(1) = 0$.
						\item<4-> if $x=-y$: $\arccos(\sclr{x}{y})=\arccos(-1) = \pi$.
					\end{itemize}
				\item<5-> If $x$ and $y$ are linearly independent, then 
					\begin{itemize}
						\item<6-> project $r$ orthogonally on $\spn\{x,y\}$ which gives us a vector $s$ with $\sclr{x}{r} = \sclr{x}{s}$ and $\sclr{y}{r} = \sclr{y}{s}$,
						\item<7-> the normalized vector $n\coloneqq s/\modul{s}$ is uniformly distributed on the intersection of the unit sphere and $\spn\{x,y\}$ by the $O(d)$-invariance of the probability distribution.
